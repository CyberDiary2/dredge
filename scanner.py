...
DOMAINS_FILE = "discovered-domains.txt"
_domains_seen = set()
...
\033[38;5;22m
  ...
  ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═════╝  ╚═════╝ ╚══════╝
\033[0m
  Drew's Reconnaissance and Enumeration Dashboard for Ghost Endpoints
  masscan + TLS cert extraction + HTTP fingerprinting
"""

def save_domain(domain):
    if domain and domain not in _domains_seen:
        _domains_seen.add(domain)
...
class SSLChecker:
    def __init__(
        self,
        ssl_ports=[443, 8443, 4443, 8080, 8888, 9443],
        ...
        for port in self.ssl_ports:
            try:
                cert = await asyncio.to_thread(
                    ssl.get_server_certificate, (ip, port), timeout=self.timeout
                )

                x509 = crypto.load_certificate(crypto.FILETYPE_PEM, cert)
                subject = x509.get_subject()
                common_name = subject.CN
                print(f"{common_name} (port {port})")
                save_domain(common_name)
    ...
    def run_masscan(self):
        try:
            ports = ",".join(str(p) for p in self.ssl_ports)
            command = f"sudo masscan -p{ports} --rate {self.masscan_rate} --wait 0 -iL {self.ips_file} -oH {self.mass_scan_results_file}"
